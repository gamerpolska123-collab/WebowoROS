    ...
    <div className="min-h-screen bg-neutral-900 text-white">
      ...
      <header className="bg-neutral-800 border-b border-neutral-700 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm text-neutral-400">{activeOrders.length} aktywnych</span>
      ...
      <main className="p-6 overflow-x-auto">
        {loading ? (
          ...
          <div className="flex gap-4 min-w-max">
            {columns.map((status) => {
              const colOrders = activeOrders.filter((o: any) => o.status === status);
              return (
                            ...
                            {order.items?.map((item: any) => (
                              <div key={item.id} className="text-sm text-neutral-700">
                                ...
                                {item.addons?.length > 0 && (
                                  <span className="text-neutral-500 text-xs"> +{item.addons.length} dod.</span>
                                )}
                              </div>
                            ))}
                          </div>
