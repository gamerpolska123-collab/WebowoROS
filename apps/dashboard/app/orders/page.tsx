...
export default function OrdersPage() {
  ...
  const handleStatusChange = useCallback(async (id: string, status: string) => {
    ...
    // Optimistic update
    if (result?.data) {
      setOptimisticOrders(result.data.map((o) => o.id === id ? { ...o, status } : o));
    }
    try {
  ...
  const handleSimulatePayment = useCallback(async (id: string) => {
    ...
    try {
      ...
      toast({ title: "Sukces", description: res.message });
      refetch();
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : "Nieznany błąd";
      toast({ title: "Błąd symulacji", description: msg, variant: "destructive" });
    } finally {
