        ...
        <div className="bg-white p-6 rounded-xl border shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">{(summary?.totalRevenue ?? 0).toFixed(2)} zł</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl border shadow-sm">
        ...
        <div className="bg-white p-6 rounded-xl border shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">{(summary?.avgOrderValue ?? 0).toFixed(2)} zł</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl border shadow-sm">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={report}>
            ...
            <YAxis tick={{ fontSize: 12 }} tickFormatter={(v) => `${v} zł`} />
            <Tooltip
              formatter={(value: number) => [`${(value ?? 0).toFixed(2)} zł`, "AOV"]}
              contentStyle={{ borderRadius: "8px", border: "1px solid #e5e7eb" }}
            />
            <Legend />
            <Line
            ...
            <tbody className="divide-y">
              {report.map((row, i) => (
                <tr key={i} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right font-medium">{(row?.revenue ?? 0).toFixed(2)} zł</td>
                  <td className="px-4 py-3 text-right">{row.orders}</td>
                  <td className="px-4 py-3 text-right">{(row?.avgOrderValue ?? 0).toFixed(2)} zł</td>
                </tr>
              ))}
            </tbody>
          </table>
