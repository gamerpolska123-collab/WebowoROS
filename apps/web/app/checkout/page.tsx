  ...
  const handleSubmit = async () => {
    ...
    try {
      const orderData = {
        ...
        deliveryEmail: deliveryForm.email,
        deliveryStreet: deliveryForm.deliveryType === "delivery" ? deliveryForm.street : undefined,
        deliveryBuildingNumber: deliveryForm.deliveryType === "delivery" ? deliveryForm.buildingNumber : undefined,
        deliveryApartmentNumber: deliveryForm.apartmentNumber || undefined,
        deliveryCity: deliveryForm.deliveryType === "delivery" ? deliveryForm.city : undefined,
        deliveryPostalCode: deliveryForm.deliveryType === "delivery" ? deliveryForm.postalCode : undefined,
        deliveryFloor: deliveryForm.floor || undefined,
        deliveryIntercom: deliveryForm.intercom || undefined,
      ...
      setErrors({ submit: err.response?.data?.message || "Wystąpił błąd podczas składania zamówienia" });
      setSubmitting(false);
    }
  };
        ...
        {step === 2 && (
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              >
                <CreditCard className={`w-6 h-6 ${paymentMethod === "card" ? "text-red-600" : "text-neutral-400"}`} />
                <div className="text-left">
                  <p className={`font-medium ${paymentMethod === "card" ? "text-red-700" : "text-neutral-900"}`}>Karta płatnicza</p>
                  <p className="text-sm text-neutral-500">Visa, Mastercard, BLIK</p>
                </div>
                {paymentMethod === "card" && <Check className="w-5 h-5 text-red-600 ml-auto" />}
              </button>
              ...
              >
                <Smartphone className={`w-6 h-6 ${paymentMethod === "blik" ? "text-red-600" : "text-neutral-400"}`} />
                <div className="text-left">
                  <p className={`font-medium ${paymentMethod === "blik" ? "text-red-700" : "text-neutral-900"}`}>BLIK</p>
                  <p className="text-sm text-neutral-500">Szybka płatność z telefonu</p>
                </div>
                {paymentMethod === "blik" && <Check className="w-5 h-5 text-red-600 ml-auto" />}
              </button>
              ...
              >
                <Banknote className={`w-6 h-6 ${paymentMethod === "cash" ? "text-red-600" : "text-neutral-400"}`} />
                <div className="text-left">
                  <p className={`font-medium ${paymentMethod === "cash" ? "text-red-700" : "text-neutral-900"}`}>Gotówka przy odbiorze</p>
                  <p className="text-sm text-neutral-500">Zapłać przy dostawie lub odbiorze</p>
                </div>
                {paymentMethod === "cash" && <Check className="w-5 h-5 text-red-600 ml-auto" />}
              </button>
            </div>
        ...
        {step === 3 && (
          <div className="space-y-6">
            ...
            <div className="flex gap-3">
              ...
              >
                {submitting ? "Składanie zamówienia..." : `Zamów i zapłać ${total.toFixed(2)} zł`}
              </button>
            </div>
          </div>
        )}
