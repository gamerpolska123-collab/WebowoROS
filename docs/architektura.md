...
> **Ważne**: System nie posiada modułu magazynowego (inventory). Dostępność produktów jest zarządzana **ręcznie** przez personel w dashboardzie (toggle `isAvailable`). Braki składników (np. zabrakło pieczarek) obsługuje kuchnia przez oznaczenie produktu jako niedostępny — bez automatycznej synchronizacji ze stanem magazynowym.
...
│ image_url │     │ image_url │     └─────────────────┘
└─────────────────┘     │ is_available │     ← RĘCZNE toggle przez personel
                        │ is_featured │         (brak modułu magazynowego)
                        │ tags[] │
                        └─────────────────┘
                               │
                               │
...
**Uwaga do `Product.isAvailable`**: Pole to jest zarządzane wyłącznie ręcznie przez personel w dashboardzie. Gdy w kuchni zabraknie składnika (np. pieczarek), kucharz lub manager oznacza produkt jako `isAvailable = false`. System **nie** monitoruje stanów magazynowych automatycznie.

### 3.2 Konfiguracja Upsellu i Rekomendacji
...
**Wydruk biletów**:
- **Bilet kuchenny** — drukowany natychmiast po złożeniu zamówienia (zawiera pozycje, uwagi, alergeny, bez cen)
- **Bilet kierowcy** — drukowany gdy kuchnia oznaczy zamówienie jako "gotowe" (zawiera pełne rozbicie cenowe, dane klienta — służy kasjerowi do przepisania na kasę fiskalną 1:1)
...
| `kitchen` | Dostęp do KDS, zmiana statusu zamówienia, oznaczanie produktów jako niedostępne |
...
### Braki składników (ręczna obsługa)
- Personel (kuchnia/manager) oznacza produkt jako niedostępny w dashboardzie (`PATCH /admin/products/:id/availability`)
- Zmiana jest broadcastowana przez WebSocket — produkt staje się szary/ukryty na stronie klienta
- Brak automatycznej synchronizacji z magazynem — to świadoma decyzja architektoniczna

---

*Architektura v1.1 — 2026-08-13*  
*Zmiany: doprecyzowanie braku modułu magazynowego, ręczne zarządzanie dostępnością, rozdzielenie biletów kuchennych i kierowcy.*
