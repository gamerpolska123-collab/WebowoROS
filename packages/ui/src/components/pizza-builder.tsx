...
import { Button, Badge } from "@ros/ui";
import { Plus, Minus } from "lucide-react";
import Image from "next/image";

interface PizzaBuilderAddon {
  addonId: string;
  name: string;
  price: number;
  quantity: number;
...
export interface PizzaBuilderConfig {
  productId: string;
  name: string;
  imageUrl?: string;
  basePrice: number;
...
interface PizzaBuilderProps {
  product: {
    ...
    variants?: { id: string; name: string; priceAdjustment: number | string }[];
    addons?: { id: string; name: string; price: number | string; maxQuantity: number }[];
  };
  onAddToCart?: (config: PizzaBuilderConfig) => void;
  onCancel?: () => void;
}

export function PizzaBuilder({ product, onAddToCart, onCancel }: PizzaBuilderProps) {
  const [selectedVariant, setSelectedVariant] = useState<
    { id: string; name: string; priceAdjustment: number | string } | null
  ...
  const variantAdjustment = selectedVariant ? Number(selectedVariant.priceAdjustment) : 0;
  const unitPrice = basePrice + variantAdjustment;
  const addonsTotal = selectedAddons.reduce((sum, a) => {
    const addon = product.addons?.find((ad) => ad.id === a.addonId);
  ...
  const handleAddonQuantity = (addonId: string, qty: number) => {
    ...
    setSelectedAddons((prev) =>
      prev.map((a) => (a.addonId === addonId ? { ...a, quantity: qty } : a))
    );
  };

  const handleAdd = () => {
    const addons: PizzaBuilderAddon[] = selectedAddons.map((a) => {
      const addon = product.addons?.find((ad) => ad.id === a.addonId);
      return {
        addonId: a.addonId,
    ...
    onAddToCart?.({
      ...
      variantPriceAdjustment: variantAdjustment || undefined,
      addons,
      quantity,
    ...
    <div className="space-y-6">
              ...
              >
                ...
                {Number(variant.priceAdjustment) > 0 && (
                  <span className="ml-1 text-red-600">
                    +{Number(variant.priceAdjustment).toFixed(2)} zł
                  </span>
                )}
              </button>
            ))}
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-red-600 font-semibold">
                      +{Number(addon.price).toFixed(2)} zł
                    </p>
                  </div>
                  {selected && (
                    <div
      ...
      <div className="flex items-center gap-4 pt-4 border-t border-neutral-100">
        ...
        <Button onClick={handleAdd} className="flex-1 py-6 text-lg font-bold rounded-full">
          Dodaj do torby — {(itemTotal ?? 0).toFixed(2)} zł
        </Button>
      </div>
    </div>
  );
