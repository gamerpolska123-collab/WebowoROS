    ...
    <div className="bg-white rounded-2xl border border-neutral-200 overflow-hidden hover:shadow-lg transition-shadow group">
        ...
        <div className="relative h-48 bg-neutral-100 overflow-hidden">
          ...
          <div className="absolute top-2 left-2 flex flex-wrap gap-1">
            {product.badges?.map((badge: any) => (
              <Badge key={badge.id} variant={badge.badgeType === "bestseller" ? "default" : "secondary"} className="text-xs">
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between">
          <div>
            <span className="text-xl font-black text-red-600">{minPrice.toFixed(2)} zł</span>
        ...
        {product.variants?.length > 0 && (
          <p className="text-xs text-neutral-400 mt-2">
            {product.variants.length} wariant{product.variants.length === 1 ? "" : "y"}
          </p>
        )}
      </div>
    </div>
