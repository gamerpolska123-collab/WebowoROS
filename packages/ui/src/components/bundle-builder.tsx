...
export function BundleBuilder({ name, discountPercent, slots, onAddBundle, className }: BundleBuilderProps) {
  ...
  const allFilled = slots.every((slot, idx) => (selections[idx] || []).length === slot.quantity);
        ...
        {slots.map((slot, idx) => (
          <div key={idx}>
            <h4 className="text-sm font-semibold text-dark mb-2">
              [{idx + 1}] {slot.label} ({(selections[idx] || []).length}/{slot.quantity})
            </h4>
              ...
              {slot.options.map((opt) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="block text-xs text-gray-500">{opt.price.toFixed(2)} zł</span>
                  </button>
                );
              })}
            </div>
        ...
        <div className="border-t pt-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <span className="ml-2 text-2xl font-bold text-primary">{discounted.toFixed(2)} zł</span>
            </div>
            <Button
              disabled={!allFilled}
